...
## 0. What changed since round 1 (an independent round-1 pass raised these; confirm they are resolved, and review the whole afresh)

- **Freshness metadata was overstated.** Round-1 wording said cache metadata
  "proves up-to-dateness". Corrected: it is at most a freshness *signal* — a
  timestamp is forgeable, an `UNCHECKED_HASH` header is not consulted at load, and
  an accepted `CHECKED_HASH` validates only header-vs-current-source, never that
  the bytecode came from it (§4a, freshness sentence).
- **Discovery scope was "shipped"-only.** Corrected to every executable artifact
  the target runtime may *select* for the candidate — in-tree, installed
  elsewhere, or resolved from an external/central cache or load path — while still
  excluding unrelated data/config (not L3) (§4a opening, §4b step 4).
- **Local-regeneration clearance was under-specified.** Corrected to name the
  toolchain/recipe, remove competing artifacts, and verify the runtime-selected
  bytes match the regenerated artifact **by digest** (§4a clearer (a)).
...
>   *signal* the runtime uses, never evidence of correspondence: a timestamp is
>   forgeable, an `UNCHECKED_HASH` header is not even consulted at load, and an
>   accepted `CHECKED_HASH` validates only that the header matches the current
>   source — never that the bytecode was produced from it. A filename or
>   "generated" claim proves nothing. An artifact the runtime may select whose
...
|---|---|---|---|
| D1 UNCHECKED-DECOY | clean decoy `.py` + UNCHECKED hash `.pyc` = payload | HIT (executed PAYLOAD) | unchecked pyc loads without consulting source (its header is not read) |
