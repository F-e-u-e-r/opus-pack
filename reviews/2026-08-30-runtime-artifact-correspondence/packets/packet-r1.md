# Design review — a correspondence limb for the source-review install gate
...
runtime does not necessarily execute that source — it may load a **shipped
compiled / cached / bundled artifact** (a `.pyc`, a minified bundle, a checked-in
`dist/`) whose bytes were never produced from the reviewed source. The reviewer
clears clean source while the interpreter executes different, unreviewed bytes.
...
>   above clears source *text*; a runtime can load a shipped compiled, bundled,
>   generated, or cached artifact (a `.pyc`, a minified bundle, a checked-in
>   `dist/`, a build cache) in preference to — or in the absence of — that
>   source, so its executable contents are cleared only when the
>   runtime-selected bytes are themselves reviewed, or an independent path
>   establishes those *exact* bytes were produced from the reviewed source under
>   a named build/compile recipe. Legitimate clearance: (a) remove the shipped
>   artifact, regenerate from the exact reviewed source with an observed
>   toolchain, and confirm the runtime selects the regenerate; (b) bind the exact
>   artifact bytes by digest to the reviewed source + recipe via
>   reproducible/attested build evidence; or (c) review the runtime-selected
>   artifact itself as the executable truth. A stable tree digest proves
>   identity, not correspondence; cache-freshness metadata — a hash-based
>   `.pyc`'s source-hash header, a timestamp — proves up-to-dateness, not that
>   the bytes came from the reviewed source; a filename or "generated" claim
>   proves nothing. An artifact the runtime may select whose correspondence you
>   cannot establish is a finding — fail closed (the opaque-dependency default);
>   a source-only candidate with no such artifact is not penalized. This
>   operationalizes security-architect's "what executes must be verifiably bound
>   to what was reviewed" at per-candidate install/vetting time.
...
>    *text*, not the bytes the runtime will load. Identify every shipped
>    compiled, bundled, generated, or cached artifact the runtime may select (a
>    `.pyc`/`.pyo`, a built bundle, a checked-in `dist/`, a cache), and apply
>    operational-rigor §2's runtime-selected-artifact correspondence gate to each
>    before proceeding. Done: each such artifact cleared by that gate or recorded
>    as a finding.
...
## 5. First-hand mechanical evidence (run just now on CPython 3.9.6, cache_tag cpython-39)
...
|---|---|---|---|
| D1 UNCHECKED-DECOY | clean decoy `.py` + UNCHECKED hash `.pyc` = payload | HIT (executed PAYLOAD) | unchecked pyc loads without consulting source |
...
| D3 CHECKED-HASH-MISMATCH | header hash = hash(other), body = payload | recompiled → DECOY; payload did NOT run | control: checked-hash *does* fire on header≠source — it is a real invalidation control, but only of freshness |
...
| D7 LOCAL-REBUILD | ship D2 forge, then remove + `py_compile` from exact source | CLEAR (executed DECOY; `regen_digest == honest_digest`) | legitimate clearance path (a): regenerate from exact source + verify selection |
| D8 VERIFIED-BUILD-PROVENANCE | reproduce artifact from reviewed source; compare digests | good `.pyc` digest == reproduction (CLEAR); forge digest ≠ reproduction (finding) | legitimate clearance path (b): digest binding; a "generated" claim alone cannot separate good from forge |
...
Prior orientation (P1–P8, same interpreter, consistent with the above): P1
unchecked-decoy → payload; P2 checked-hash mismatch → recompile; P3 timestamp
invalidation control holds; P4 timestamp collision → stale payload; P5 source-only
→ source; P6 digest proves identity not semantics (two stable reads, anomalies=[],
one byte-flip moves it); plus a source-less bonus and an out-of-tree central-cache
observation (env-specific: identity coverage can be zero, strengthening
"correspondence must be established at the runtime-selection point, not the
tree-identity point").
...
5. Does the **local-regeneration** clearance require verifying source AND build
   path AND runtime selection (not just recompiling)?
...
9. Is the rule kept at **L2** and NOT expanded into an L3 all-supply-chain /
   all-runtime-reachable-artifact totality?
