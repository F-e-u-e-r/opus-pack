# Issue-115 STAGE-2 State-Transition Table (r4)
...
no completion object (transport/API failure), OR a recorded protocol
mismatch with its evidence attached. An evidence-free INVALID label
...
| 23d | SUSPECT-RERUN-PENDING [M] | owner cancels BEFORE row 23a begins for this unit (i.e. before its prerequisite smoke or first R-slot, whichever would come first — after that, cancellation does not exist: no output, smoke output included, can inform one) | — | marker returns to SUSPECT (restore/demote only — rerun-used stays consumed) | cancellation receipt | none (nothing encumbered) |
