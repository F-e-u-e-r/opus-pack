...
issue115-t2probe-v1 prereg package. Deterministic; re-run to
re-derive. Read-only over artifact files; writes only the three
generated outputs.

Same immutability model as issue115-stage2-v1: once an
OWNER-APPROVAL.json binds this package id as APPROVED, regeneration
is refused — a new version means a new PACKAGE_ID."""
import hashlib, json, os, sys

ROOT = os.path.dirname(os.path.abspath(__file__))
PACKAGE_ID = "issue115-t2probe-v1"
BASELINE_MAIN = "b68a4fb25b796a9bcf4be9eb7aa8d45de322cc19"
...
def rendered(fid):
    ...
    add = read("wrappers/clauses/EXPLICIT-CONTROL-addendum.txt")
    b = fx
    c = PREAMBLE + cl + MID + fx
    e = PREAMBLE + cl + ADD_SEP + add + MID + fx
    return {k: hashlib.sha256(v).hexdigest() for k, v in
            (("B", b), ("C", c), ("E", e))}
...
def main():
    ...
    for fid, pos, rubric in FIXTURES:
        entries.append({
            ...
            "addendum_file": "wrappers/clauses/EXPLICIT-CONTROL-addendum.txt",
            "addendum_sha256": sha_path("wrappers/clauses/EXPLICIT-CONTROL-addendum.txt"),
            "rubric": rubric,
            "rubric_sha256": sha_path(rubric),
            "predicate": "rubrics/ORDINAL-PREDICATE.md",
            "predicate_sha256": sha_path("rubrics/ORDINAL-PREDICATE.md"),
    ...
    manifest = {
        ...
        "sealed_anchors": {
            "T2S2_content_sha256": sha_path("fixtures/P1.md"),
            "T2_clause_sha256": sha_path("wrappers/clauses/CURRENT-clause.txt"),
        ...
        "documents": {p: sha_path(p) for p in [
            "T2-PROBE-PREREG-v1.md", "SMOKE-CHECKLIST.md",
