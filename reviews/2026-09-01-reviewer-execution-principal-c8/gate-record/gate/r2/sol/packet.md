...
| E4 INDEPENDENT-COPY+WRITE | write-capable critic | Stays legal (current path); the copy is a plane-2 disposable workspace — mutating it never moves the settled baseline the verdict binds to; its reach is recorded in plane 1; a workspace-writable *default* is recorded reach, never an entitlement. |
...
| E12 CAPABILITY-DISCLOSURE | verdict records envelope? | The two-plane receipt **is** E12; current banners already assert model + effort + sandbox mode first-hand — the receipt extends this to the remaining fields. |
