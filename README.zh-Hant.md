...
- **openai/codex-security · Mapleeeeeeeeeee/cc-session-reader** — [`openai/codex-security`](https://github.com/openai/codex-security) 與 [`Mapleeeeeeeeeee/cc-session-reader`](https://github.com/Mapleeeeeeeeeee/cc-session-reader)(皆 Apache-2.0);2026-07-31 的雙 repo 挖礦掃描(十個 agent 逐字掃描、跨兩個模型家族、再加第三家族交叉檢查;所有採納主張均對原文逐條核實)。只採意念、未取文字。自安全掃描產品的 threat model 與內附審查 doctrine:子程序環境最小化、policy 形資料層、severity 綁證據、系統範圍 threat model、對外披露的讀眾檢查(`security-architect`);以及 blocked 時必須具名拒絕的假完成形態、收斂只是搜索證據、缺席不等於已解決、消費者位置驗證(`delegation-and-review`、`cross-model-review`、`operational-rigor`)。自 transcript 壓縮工具的決策記錄:按可重建性分配保留、同錯誤才可合併、可執行的省略標記、壓縮稽查路徑(`delegation-and-review` §5)。
